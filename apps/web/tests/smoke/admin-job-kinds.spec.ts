...
// Workflows at startup, so these always have data to work with —
...
test.describe('Admin Job kinds list', () => {
  test('renders rows', async ({ page }) => {
    await mountPage(page, '/system/workflows', { titleMatch: /job kind/i });
  ...
  test('row link navigates to /system/workflows/{kind}', async ({ page }) => {
    await mountPage(page, '/system/workflows', { titleMatch: /job kind/i });
    const firstLink = page.locator('a[href*="/system/workflows/"]').first();
    await expect(firstLink).toBeVisible({ timeout: 10_000 });
    await Promise.all([
      page.waitForURL(/\/admin\/workflows\/[^/]+/, { timeout: 10_000 }),
      firstLink.click(),
    ]);
  });
});
...
test.describe('Admin Job kind detail — controls', () => {
  test('Edit + Retire buttons render with correct enabled state', async ({
    ...
    await mountPage(page, '/system/workflows/ad-hoc');
    await expect(page.locator('h1')).toBeVisible({ timeout: 10_000 });

    // D6 removed the direct "Publish draft" control — authoring/publish
    // now runs through a workflow-design Job (opened via Edit…).
  ...
  test('Fork navigates to /system/workflows/new?fork=…', async ({ page }) => {
    await mountPage(page, '/system/workflows/ad-hoc');
    await expect(page.locator('h1')).toBeVisible({ timeout: 10_000 });

    await Promise.all([
      page.waitForURL(/\/admin\/workflows\/new\?fork=ad-hoc/, {
        timeout: 10_000,
      }),
