...
use crate::calendar::us_banking::is_us_federal_holiday;
...
/// when unspecified.
///
/// US federal holidays use `weekend_multiplier`. The brewery's
...
pub fn effective_rate_for_day(jr: &JobRate, today: NaiveDate) -> f64 {
    let base = jr
        .ramp
        .iter()
    ...
    let weekday_mult = if is_non_business_day(today) {
...
pub fn count_jobs_for_day(jr: &JobRate, today: NaiveDate, rng: &mut Rng) -> u32 {
    count_jobs_for_tick(jr, today, &Tick::day(), rng)
}
...
/// `e^−λ`).
pub fn count_jobs_for_tick(jr: &JobRate, today: NaiveDate, tick: &Tick, rng: &mut Rng) -> u32 {
    if jr.deterministic {
        return deterministic_count_for_tick(jr, today, tick);
    }
    let lambda = effective_rate_for_day(jr, today) * tick.day_fraction();
    if lambda <= 0.0 {
        return 0;
    }
...
pub fn deterministic_count_for_tick(jr: &JobRate, today: NaiveDate, tick: &Tick) -> u32 {
    if !tick.is_first_in_day {
        return 0;
    }
    let effective = effective_rate_for_day(jr, today);
    if effective <= 0.0 {
        return 0;
    }
...
fn is_non_business_day(day: NaiveDate) -> bool {
...
mod tests {
    ...
    fn no_ramps_returns_base_rate() {
        let r = effective_rate_for_day(&jr(2.0), date(2026, 1, 5));
    ...
    fn ramps_take_effect_at_their_date() {
        ...
        assert_eq!(effective_rate_for_day(&j, date(2026, 1, 1)), 1.0);
        assert_eq!(effective_rate_for_day(&j, date(2026, 3, 31)), 1.0);
        // After 2nd ramp
        assert_eq!(effective_rate_for_day(&j, date(2026, 4, 1)), 2.5);
        assert_eq!(effective_rate_for_day(&j, date(2026, 6, 30)), 2.5);
        // After 3rd
        assert_eq!(effective_rate_for_day(&j, date(2026, 12, 1)), 4.0);
    }
    ...
    fn before_any_ramp_falls_back_to_base_rate() {
        ...
        assert_eq!(effective_rate_for_day(&j, date(2026, 1, 1)), 0.5);
        // On the ramp date — the ramp wins.
        assert_eq!(effective_rate_for_day(&j, date(2026, 6, 1)), 5.0);
    }
    ...
    fn weekday_multiplier_only_applies_mon_fri() {
        ...
        assert_eq!(effective_rate_for_day(&j, date(2026, 1, 5)), 0.5);
        // 2026-01-03 is Saturday
        assert_eq!(effective_rate_for_day(&j, date(2026, 1, 3)), 2.0);
        // 2026-01-04 is Sunday
        assert_eq!(effective_rate_for_day(&j, date(2026, 1, 4)), 2.0);
    }
    ...
    fn us_federal_holiday_uses_weekend_multiplier() {
        ...
        assert_eq!(effective_rate_for_day(&j, date(2026, 7, 3)), 0.0);
        // 2026-12-25 (Friday) is Christmas — same.
        assert_eq!(effective_rate_for_day(&j, date(2026, 12, 25)), 0.0);
        // 2026-07-02 (Thursday) is a normal weekday → 2.0.
        assert_eq!(effective_rate_for_day(&j, date(2026, 7, 2)), 2.0);
    }
    ...
    fn evening_facing_flow_still_fires_on_holiday() {
        ...
        assert_eq!(effective_rate_for_day(&j, date(2026, 12, 25)), 1.75);
    }
    ...
    fn month_multiplier_stacks_on_top_of_weekday() {
        ...
        assert!((effective_rate_for_day(&j, date(2026, 7, 6)) - 6.0).abs() < 1e-9);
        // January 2026-01-03 is Saturday → 1.0 * 0.5 * 0.5 = 0.25
        assert!((effective_rate_for_day(&j, date(2026, 1, 3)) - 0.25).abs() < 1e-9);
        // June 2026-06-15 is Monday, no June multiplier → 1.0 * 2.0 = 2.0
        assert!((effective_rate_for_day(&j, date(2026, 6, 15)) - 2.0).abs() < 1e-9);
    }

    #[test]
    fn count_jobs_zero_rate_returns_zero() {
        let mut rng = Rng::new(42);
        let count = count_jobs_for_day(&jr(0.0), date(2026, 1, 1), &mut rng);
        assert_eq!(count, 0);
    }
    ...
    fn count_jobs_is_deterministic_for_a_seed() {
        ...
        assert_eq!(
            count_jobs_for_day(&j, day, &mut rng_a),
            count_jobs_for_day(&j, day, &mut rng_b)
        );
    }
    ...
    fn count_jobs_average_tracks_rate_across_a_year() {
        ...
        for _ in 0..365 {
            total += count_jobs_for_day(&j, day, &mut rng) as u64;
    ...
    fn deterministic_fires_exactly_one_per_weekday() {
        ...
        for offset in 0..5 {
            ...
            assert_eq!(
                count_jobs_for_day(&dr, day, &mut rng),
                1,
    ...
    fn deterministic_skips_weekends() {
        ...
        let dr = daily_review();
        let mut rng = Rng::new(7);
        assert_eq!(count_jobs_for_day(&dr, date(2026, 1, 3), &mut rng), 0);
        assert_eq!(count_jobs_for_day(&dr, date(2026, 1, 4), &mut rng), 0);
    }
    ...
    fn deterministic_skips_us_federal_holidays() {
        ...
        let dr = daily_review();
        let mut rng = Rng::new(7);
        assert_eq!(count_jobs_for_day(&dr, date(2026, 7, 3), &mut rng), 0);
        assert_eq!(count_jobs_for_day(&dr, date(2026, 12, 25), &mut rng), 0);
        // A normal weekday around them still reviews.
        assert_eq!(count_jobs_for_day(&dr, date(2026, 7, 2), &mut rng), 1);
    }
    ...
    fn deterministic_is_day_anchored_at_sub_day_ticks() {
        ...
        assert_eq!(
            count_jobs_for_tick(&dr, monday, &Tick::hour(0.0, true), &mut rng),
            1
        );
        // Every other hourly tick of the day fires nothing.
        let mut rest = 0u32;
        for h in 1..24 {
            rest += count_jobs_for_tick(&dr, monday, &Tick::hour(h as f64, false), &mut rng);
        }
    ...
    fn deterministic_week_totals_five_reviews() {
        ...
        for _ in 0..7 {
            for h in 0..24 {
                total += count_jobs_for_tick(&dr, day, &Tick::hour(h as f64, h == 0), &mut rng);
            }
            day = day.succ_opt().unwrap();
        }
